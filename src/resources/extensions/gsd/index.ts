...
const MILESTONE_CONTEXT_RE = /M(?:-[a-z0-9]{6}-)?\d+-CONTEXT\.md$/;
...
async function buildGuidedExecuteContextInjection(prompt: string, basePath: string): Promise<string | null> {
  const executeMatch = prompt.match(/Execute the next task:\s+(T\d+)\s+\("([^"]+)"\)\s+in slice\s+(S\d+)\s+of milestone\s+(M(?:-[a-z0-9]{6}-)?\d+)/i);
  if (executeMatch) {
  ...
  const resumeMatch = prompt.match(/Resume interrupted work\.[\s\S]*?slice\s+(S\d+)\s+of milestone\s+(M(?:-[a-z0-9]{6}-)?\d+)/i);
  if (resumeMatch) {
