...
| D017 | M001/S05 | api | createWorktree sync→async signature change | Made createWorktree async returning Promise<WorktreeInfo> | Required for dynamic import of gsd-db.js (D003/D014 pattern). Only one call site in worktree-command.ts which is already async. Updated 2 test files with await. | No |
