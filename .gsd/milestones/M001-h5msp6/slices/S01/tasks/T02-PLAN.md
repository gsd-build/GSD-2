...
2. Each test creates a temp dir with `.gsd/milestones/M001-h5msp6/` structure, writes a `M001-h5msp6-SECRETS.md` manifest file, calls `getManifestStatus(tmpDir, "M001-h5msp6")`, and asserts the result. Use `process.env` manipulation for env-presence tests (save/restore in try/finally).
