...
**Demo:** `getManifestStatus(base, "M001-h5msp6")` returns a categorized status object with `pending`, `collected`, `skipped`, and `existing` arrays. A realistic LLM-style manifest round-trips through `parseSecretsManifest() → formatSecretsManifest() → parseSecretsManifest()` with semantic equality.
