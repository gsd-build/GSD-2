...
1. Create `src/resources/extensions/gsd/tests/auto-secrets-gate.test.ts` following the pattern from `manifest-status.test.ts` (temp dirs, real `.gsd/milestones/M001-h5msp6/` structure, cleanup in finally blocks).
2. Write three test cases:
   - **No manifest exists**: Call `getManifestStatus(base, 'M001-h5msp6')` on a base with no `M001-h5msp6-SECRETS.md` → returns `null`. Proves the gate's null-check path.
