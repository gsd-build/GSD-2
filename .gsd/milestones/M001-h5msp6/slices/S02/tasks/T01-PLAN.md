...
**Milestone:** M001-h5msp6

## Description

S01's `getManifestStatus()`, `ManifestStatus` type, and contract tests live on the `gsd/M001-h5msp6/S01` branch but haven't been merged to this branch. The orchestrator function planned for T03 depends on these. This task merges S01, verifies the merge is clean, and creates the test file for S02 with initially-failing assertions that target the functions built in T02–T03.

## Steps

1. Merge the `gsd/M001-h5msp6/S01` branch into the current `gsd/M001-h5msp6/S02` branch. Resolve any conflicts (the diff is 4 files, 525 insertions — types.ts, files.ts, and test files).
...
- `gsd/M001-h5msp6/S01` branch — commits `93c0852` and `05ff6c6` containing `ManifestStatus` type, `getManifestStatus()` function, and contract tests
...
- Clean merge commit on `gsd/M001-h5msp6/S02` branch
