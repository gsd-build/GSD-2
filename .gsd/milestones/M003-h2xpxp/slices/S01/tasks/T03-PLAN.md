...
1. Set up test: create temp directory, `git init`, initial commit, create `.gsd/milestones/M003-h2xpxp/M003-h2xpxp-CONTEXT.md`, commit planning files.
2. Test create: `createAutoWorktree(base, "M003-h2xpxp")`. Assert worktree dir exists at `.gsd/worktrees/M003-h2xpxp/`, git branch is `milestone/M003-h2xpxp`, planning files are present in worktree, `isInAutoWorktree` returns true, `getAutoWorktreeOriginalBase()` returns original path.
3. Test teardown: create a file in worktree, commit. `teardownAutoWorktree(originalBase, "M003-h2xpxp")`. Assert `process.cwd()` back to original, worktree dir removed, `isInAutoWorktree` returns false.
...
5. Test coexistence: create auto-worktree `milestone/M003-h2xpxp` + manual worktree `worktree/explore` — both exist, no branch conflicts.
