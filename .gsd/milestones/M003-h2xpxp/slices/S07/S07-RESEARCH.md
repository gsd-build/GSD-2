...
|---------|------------------|------------|
| Temp git repo setup | `createTestContext` + helper pattern from all M003-h2xpxp tests | Consistent assertion API, cleanup |
...
- Running `npm run test:unit` across all 64 test files may surface pre-existing failures unrelated to M003-h2xpxp. These should be noted but not blocked on.
