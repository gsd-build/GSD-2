...
/**
 * GSD Visual Postinstall
 *
 * Wraps the multi-step postinstall process with clean visual output.
 * Suppresses npm/subprocess noise and shows spinner → checkmark progress.
 * On failure, captures and displays the relevant error output.
 */
...
const SILENT = process.env.GSD_POSTINSTALL_SILENT === '1'
...
function startSpinner(label) {
  if (!process.stdout.isTTY || SILENT) return
  spinnerFrame = 0
...
function printBanner() {
  ...
  // Write to stdout — npm suppresses stderr from postinstall scripts by
  // default but passes stdout through, so users see the banner without
  // needing --foreground-scripts.
...
${c.cyan}   ██████╗ ███████╗██████╗
  ...
  ${c.bold}Get Shit Done${c.reset} ${c.dim}v${version}${c.reset}
`)
}
...
function linkWorkspacePackages() {
  ...
  if (!existsSync(scriptPath)) {
    printSkip('Workspace packages', 'not applicable')
    return
  }

  startSpinner('Linking workspace packages...')
  try {
    ...
    if (result.status !== 0) {
      ...
      printWarn('Workspace packages', stderr || 'link script exited non-zero')
      return
    }
    ...
    stopSpinner()
    printWarn('Workspace packages', err.message)
  }
}

// ── Step 2: Ensure workspace builds ────────────────────────────────────────

function ensureWorkspaceBuilds() {
  const scriptPath = join(cwd, 'scripts', 'ensure-workspace-builds.cjs')
  if (!existsSync(scriptPath)) {
    printSkip('Workspace builds', 'not applicable')
    return
  }

  // This script skips itself in CI and when packages/ doesn't exist
  const packagesDir = join(cwd, 'packages')
  if (!existsSync(packagesDir) || IS_CI) {
    return // Script would skip anyway — don't show noise
  }

  // Skip when running inside node_modules (end-user install from tarball).
  // The tarball ships pre-built dist/ alongside src/ — rebuilding inside
  // node_modules hangs because npm run build isn't available there.
  if (cwd.includes('node_modules')) {
    return
  }

  startSpinner('Checking workspace builds...')
  try {
    const result = spawnSync(process.execPath, [scriptPath], {
      cwd,
      stdio: ['ignore', 'pipe', 'pipe'],
    ...
    if (result.status !== 0) {
      const stderr = (result.stderr || '').toString().trim()
      printWarn('Workspace builds', stderr || 'build check failed')
      return
    }

    const stderr = (result.stderr || '').toString()
    const buildMatch = stderr.match(/Building (\d+) workspace/)
    if (buildMatch) {
      // Check how many succeeded
      const successes = (stderr.match(/✓/g) || []).length
      const failures = (stderr.match(/✗/g) || []).length
      if (failures > 0) {
        printWarn('Workspace builds', `${successes} built, ${failures} failed`)
      } else {
        printStep('Workspace builds', `${successes} package${successes !== 1 ? 's' : ''} compiled`)
      }
    } else {
      printStep('Workspace builds', 'up to date')
    }
  } catch (err) {
    stopSpinner()
    printWarn('Workspace builds', err.message)
  }
}
...
async function installPlaywright() {
  if (PLAYWRIGHT_SKIP) {
    printSkip('Chromium', 'PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD=1')
    return
  }
  ...
  try {
    const result = await new Promise((res) => {
      execCb('npx playwright install chromium', { cwd, timeout: 300_000 }, (error, stdout, stderr) => {
    ...
    if (!result.ok) {
      // Extract the meaningful error from Playwright's output
      const output = (result.stderr + '\n' + result.stdout).trim()
      const meaningful = output.split('\n')
        .filter(l => !l.includes('npm warn') && !l.includes('npm WARN') && l.trim())
        .slice(-3)
        .join('; ')
      printWarn('Chromium', meaningful || 'install failed (run npx playwright install chromium manually)')
      return
    }
...
async function installRtk() {
  if (RTK_SKIP) {
    printSkip('RTK', 'disabled via env')
    return
  }
