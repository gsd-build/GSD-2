...
在无头自动模式下运行时，远程提问允许 GSD 通过 Slack、Discord 或 Telegram 请求用户输入，并发送信息型通知。当 GSD 遇到需要人工判断的决策点时，它会把问题发到你配置好的频道，并轮询等待响应。milestone 完成、阻塞、预算预警以及其他状态事件，也会被发送到同一个频道。
