...
当 Telegram 被配置为远程频道时，你可以直接在 Telegram 聊天中控制自动模式、查询项目状态，而不必回到终端操作。

| 命令 | 作用 |
|------|------|
| `/pause` | 在当前单元完成后暂停自动模式 |
| `/resume` | 清除暂停指令并继续自动模式 |
| `/status` | 显示当前 milestone、活跃单元和当前会话成本 |
| `/progress` | 查看 roadmap 概览（已完成 / 未完成 milestones） |
| `/budget` | 查看当前会话的 token 使用量和成本 |
| `/log [n]` | 查看最近 `n` 条活动日志（默认 5） |

当自动模式处于运行状态时，GSD 会大约每 5 秒轮询一次 Telegram 入站命令。只有在自动模式活动期间，这些命令才可用。

完整命令列表和配置方式见 [远程提问 - Telegram 命令](./remote-questions.md#telegram-命令)。
